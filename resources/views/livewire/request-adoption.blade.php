<div>
    ...
    @if ($showForm)
        ...
        <form wire:submit.prevent="submit" class="max-w-3xl bg-white mx-auto border-2 border-gray-500 rounded-3xl p-4 md:p-8">
            <h1 class="text-xl text-center font-poppinsBlack py-4">Adoptar a: {{ $pet->name }}</h1>
            <img src="{{ $pet->getFirstMediaUrl('pets') }}" alt="{{ $pet->name }}" class="mx-auto w-40 bg-white rounded-lg">
    ...
    @else
        <div class="max-w-3xl bg-white mx-auto border-2 border-gray-500 rounded-3xl p-4 md:p-8">
            ...
            <p class="text-center p-6 bg-green-300 rounded border-2 border-green-500">Ya has enviado una solicitud de adopción de la mascota, en breve nos pondremos en contacto contigo.</p>
        </div>
    @endif
